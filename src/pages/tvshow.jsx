...
import Recommendation1 from "../components/recommendation.1";
...
class TvShow extends Component {
  constructor(props) {
    ...
    if (this.props.location.state) {
      ...
      let url2 = `https://api.themoviedb.org/3/tv/${movieId}/content_ratings?api_key=890fb371f22db088105d415f5cafd470&language=en-US`;

      let url3 = `https://api.themoviedb.org/3/tv/${movieId}/credits?api_key=890fb371f22db088105d415f5cafd470`;

      let url4 = `https://api.themoviedb.org/3/tv/${movieId}/keywords?api_key=890fb371f22db088105d415f5cafd470`;
      ...
      let url5 = `https://api.themoviedb.org/3/tv/${movieId}/videos?api_key=890fb371f22db088105d415f5cafd470`;
      fetch(url5)
        .then(response => response.json())
        .then(this.buildList5)
        .catch();

      let url6 = `https://api.themoviedb.org/3/tv/${movieId}/recommendations?api_key=890fb371f22db088105d415f5cafd470&language=en-US&page=1`;
      fetch(url6)
        .then(response => response.json())
  ...
  buildList4 = data => {
    ...
    this.setState({ keyword: data.results });
  };
  ...
  render() {
    var year = String(this.state.list.first_air_date).split("-", 1)[0];
    ...
    if (this.props.location.state) {
      return (
        <div className="App movie">
          ...
          <div
            style={{
              backgroundImage: `linear-gradient(rgba(57, 53, 52, 0.80), rgba(57, 53, 52, 0.80)), url(https://image.tmdb.org/t/p/w1000_and_h450_multi_faces${this.state.list.backdrop_path})`
            }}
            className="movieTop"
          >
            ...
            <div className="movieTopDetail">
              ...
              {this.state.list.first_air_date ? (
                <div className="movieTopRelease">
                  <span>
                    {this.state.release} {this.state.list.first_air_date}{" "}
                  </span>
                  <FontAwesomeIcon
                    style={{
                      fontSize: "6px",
                  ...
                  <FontAwesomeIcon
                    ...
                    size="xs"
                  />
                  <span> {this.state.duration && this.state.duration} </span>
                </div>
              ) : (
                <div> </div>
              )}
              ...
              {this.state.list.overview ? (
                <React.Fragment>
                  ...
                  <div className="crew">
                    <div className="crew1">
                    <div> {crew1Name == "-" && ( this.state.list.created_by[0] ? ( this.state.list.created_by[0].name ) : "-" )} </div>
                      <div> {crew1Job == "-" && ( this.state.list.created_by[0] ? "Creator" : "-" ) }</div>{" "}
                    </div>
                    <div className="crew2">
                    <div> {crew1Name == "-" && ( this.state.list.created_by[1] ?  (this.state.list.created_by[1].name ) : "-") } </div>
                      <div> {crew1Job == "-" && ( this.state.list.created_by[1] ? "Creator" : "-" ) }</div>{" "}
                    </div>
                    <div className="crew3">
                    <div> {crew1Name == "-" && ( this.state.list.created_by[2] ?  (this.state.list.created_by[2].name ) : "-") } </div>
                      <div> {crew1Job == "-" && ( this.state.list.created_by[2] ? "Creator" : "-" ) }</div>{" "}
                    </div>
                  </div>
                </React.Fragment>
              ) : (
              ...
              {this.state.video.results ? (
                <div className="MBI-recommendation">
                  ...
                  <div onClick={this.recrender1} className="MBI-recCard">
                    <Recommendation1 reclist={this.state.rec} />
                  </div>
                </div>
              ) : (
                <Skeleton
